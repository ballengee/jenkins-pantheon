...
if (!class_exists('\Wpo\Services\Authentication_Service')) {
    ...
    class Authentication_Service
    {
        ...
        private static function skip_authentication()
        {
            ...
            if (!empty($error_page_url) && stripos($error_page_url, rtrim($GLOBALS['WPO_CONFIG']['url_info']['wp_site_url'], '/')) === 0) {
            ...
            if (!is_admin() && !is_network_admin() && ($scenario === 'internet' || $scenario === 'internetAuthOnly')) {
                ...
                foreach ($private_pages as $private_page) {
                    ...
                    if ((!empty($login_urls['default_login_url']) && stripos($private_page, $login_urls['default_login_url']) !== false) || (!empty($login_urls['custom_login_url']) && stripos($private_page, $login_urls['custom_login_url']) !== false)) {
