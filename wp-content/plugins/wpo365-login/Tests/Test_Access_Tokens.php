...
if (!class_exists('\Wpo\Tests\Test_Access_Tokens')) {
    ...
    class Test_Access_Tokens
    {
        ...
        {
            $test_result = new Test_Result('Send email from WordPress using Microsoft Graph.', Test_Result::CAPABILITY_MAIL, Test_Result::SEVERITY_LOW);
            $test_result->passed = true;
        ...
        public function test_sync_cron()
        {
            ...
            foreach ($allowed_urls as $url) {
                ...
                if (false !== \stripos($url, 'wp-cron.php')) {
                    $found = true;
                    break;
                }
            }
        ...
        public function test_sync_delegated()
        {
            ...
            // Check if default domain is configured
            ...
            foreach ($allowed_endpoints_and_permissions as $allowed_endpoint_config) {
                ...
                if (stripos($allowed_endpoint_config['key'], '/users') > 0) {
                    $users_endpoint_ok = true;
                    continue;
                }

                if (stripos($allowed_endpoint_config['key'], '/myorganization') > 0) {
        ...
        public function test_sync_application()
        {
            ...
            for ($i = 0; $i < sizeof($allowed_endpoints_and_permissions); $i++) {
                ...
                if (stripos($allowed_endpoints_and_permissions[$i]['key'], '/users') > 0) {
                    $users_endpoint_ok = true;
                ...
                if (stripos($allowed_endpoints_and_permissions[$i]['key'], '/myorganization') > 0) {
                    $myorganization_endpoint_ok = true;
        ...
        public function test_sharepoint_search_delegated()
        {
            ...
            foreach ($allowed_endpoints_and_permissions as $allowed_endpoint_config) {
                ...
                if (stripos($allowed_endpoint_config['key'], 'sharepoint.com') > 0 && false === $allowed_endpoint_config['boolVal']) {
        ...
        public function test_sharepoint_documents_delegated()
        {
            ...
            foreach ($allowed_endpoints_and_permissions as $allowed_endpoint_config) {
                ...
                if (stripos($allowed_endpoint_config['key'], '/sites') > 0) {
                    $sites_endpoint_ok = true;
                    continue;
                }

                if (stripos($allowed_endpoint_config['key'], '/drives') > 0) {
        ...
        private function check_static_permission($access_token, $permission, $permission_type, $static_permissions, $severity, $category = Test_Result::CAPABILITY_CONFIG, $additional_message = '', $more_info = 'https://docs.wpo365.com/article/23-integration')
        {
            ...
            if ($this->use_b2c && strcasecmp($permission, 'mail.send') === 0) {
            ...
            foreach ($static_permissions as $key => $static_permission) {
                ...
                if (false !== stripos($static_permission, $permission)) {
                    return $test_result;
                }
            }
