...
class Plugin_Updater
{
     ...
     */
    public function __construct($_api_url, $_plugin_file, $_api_data = null, $slug = '')
    {
    ...
    public static function check_for_updates()
    {
        ...
        // Current page is not plugins.php and transient helper is not expired
        if ('plugins.php' != $pagenow && false !== Wpmu_Helpers::mu_get_transient('wpo365_check_for_updates')) {
            return;
        }

        // Create a transient that will expire after 4 hours
        Wpmu_Helpers::mu_set_transient('wpo365_check_for_updates', time(), 14400);

        // Refresh all license related admin notices
    ...
    public static function check_license($extension, $license_key)
    {
        ...
        if (empty($extension) || !$extension['activated']) {
            return;
        }
        ...
        if (false !== stripos($url, 'localhost')) {
            ...
            foreach ($skip_list as $skip) {
                ...
                if (false !== stripos($url_path, $skip)) {
                    $skip_license_check = true;
                    break;
                }

                if (false !== stripos($url_subdomain, $skip)) {
                    $skip_license_check = true;
                    break;
                }
            }
        ...
        if (is_wp_error($response) || 200 !== wp_remote_retrieve_response_code($response)) {
            ...
            Log_Service::write_log('WARN', __METHOD__ . ' -> ' . $message);
        } else {
            ...
            if (false === $license_data->success) {
                ...
                switch ($license_data->error) {
                    ...
                    case 'missing':
                        ...
                        $message =
                            sprintf(
                                __('Your license key for <strong>%s</strong> is missing. Please go to <a href="%s" target="_blank">WP Admin > WPO365 > Licenses</a> and update your license or purchase a <a href="%s" target="_blank">new license online</a>. See the <a href="%s" target="_blank">End User License Agreement</a> for details'),
                                $extension['store_item'],
                                $lic_url,
                                $extension['store_url'],
                                'https://www.wpo365.com/end-user-license-agreement/'
                    ...
                    case 'site_inactive':
                        ...
                        $message =
                            sprintf(
                                __('Your license key for <strong>%s</strong> is not activated for this URL. Please go to <a href="%s" target="_blank">WP Admin > WPO365 > Licenses</a> and activate your license or purchase a <a href="%s" target="_blank">new license online</a>. See the <a href="%s" target="_blank">End User License Agreement</a> for details'),
                                $extension['store_item'],
                                $lic_url,
                                $extension['store_url'],
                                'https://www.wpo365.com/end-user-license-agreement/'
                    ...
                    case 'no_activations_left':
                        ...
                        $message =
                            sprintf(
                                __('Your license key for <strong>%s</strong> has reached its activation limit. Please go to <a href="%s" target="_blank">WP Admin > WPO365 > Licenses</a> and activate your license or purchase additional <a href="%s" target="_blank">licenses online</a>. See the <a href="%s" target="_blank">End User License Agreement</a> for details'),
                                $extension['store_item'],
                                $lic_url,
                                $extension['store_url'],
                                'https://www.wpo365.com/end-user-license-agreement/'
                    ...
                    default:
                        ...
                        $message =
                            sprintf(
                                __('An unknown error occurred whilst checking your license key for %s. Please try again.'),
                                $extension['store_item']
                            );
                        break;
                }
                ...
                if (!empty($message)) {
                    $lic_notices[] = \sprintf('Could not find a license for %s. Please go to WP Admin > WPO365 > Licenses and activate your license or purchase a <a href="%s">new license online</a>', $extension['store_item'], $extension['store_url']);
                    Wpmu_Helpers::mu_set_transient('wpo365_lic_notices', $lic_notices);
                    return;
                }
            }
    ...
    public function show_update_notification($file, $plugin)
    {
        ...
        if (!empty($update_cache->response[$this->name]) && version_compare($this->version, $version_info->new_version, '<')) {
            ...
            $changelog_link = self_admin_url('index.php?edd_sl_action=view_plugin_changelog&plugin=' . esc_html($this->name) . '&slug=' . esc_html($this->slug) . '&TB_iframe=true&width=772&height=911');
    ...
    private function api_request($_action, $_data)
    {
        ...
        if ($this->api_url == trailingslashit($url)) {
