...
if (!class_exists('\Wpo\Core\Url_Helpers')) {
    ...
    class Url_Helpers
    {
        ...
        public static function is_wp_rest_api()
        {
            ...
            $rest_url_wo_protocol = \substr($rest_url, \stripos($rest_url, '://') + 3);

            $current_url = $GLOBALS['WPO_CONFIG']['url_info']['current_url'];
            $current_url_wo_protocol = \substr($current_url, \stripos($current_url, '://') + 3);

            if (stripos($current_url_wo_protocol, $rest_url_wo_protocol) === 0) {
                return true;
            }
        ...
        public static function is_wp_login($uri = NULL)
        {
            ...
            $default_login_url_detected = !empty($default_login_url_path) && false !== stripos($uri,  $default_login_url_path);
        ...
        public static function force_redirect($url)
        {
            ...
            if (false === strpos($location, '?') && false === strpos($location, '#')) {
        ...
        public static function goto_after($wpo_usr)
        {
            ...
            $cookie_domain_host = defined('COOKIE_DOMAIN') && false !== COOKIE_DOMAIN ? ltrim(COOKIE_DOMAIN, '.') : '';
            ...
            $log_level = stripos($state_url_host, 'teams.microsoft.com') === 0 ? 'WARN' : 'ERROR';
            ...
            if (
                ...
                false === (!empty($cookie_domain_host) && false !== stripos($aad_redirect_url_host, $cookie_domain_host) && false !== stripos($wp_site_url_host, $cookie_domain_host))
            ) {
             ...
             */
            elseif (!empty($cookie_domain_host) && false === stripos($wp_site_url_host, $cookie_domain_host)) {
            ...
            elseif (
                ...
                false === (!empty($cookie_domain_host) && false !== stripos($state_url_host, $cookie_domain_host) && false !== stripos($aad_redirect_url_host, $cookie_domain_host))
            ) {
            ...
            elseif (
                ...
                false === (!empty($cookie_domain_host) && false !== stripos($wp_site_url_host, $cookie_domain_host) && false !== stripos($state_url_host, $cookie_domain_host))
            ) {
