...
use NinjaForms\Salesforce\Common\Entities\LogEntry;
...
class TableLogHandler implements NfLogHandler
{
    ...
    /**
     * Get log entries by timestamp
     * 
     * @param string $start The datetime that marks the start point of the search
     *                      Formatted: "Y-m-d H:i:s"
     * @param string $end   The datetime that marks the end point of the search
     *                      Formatted: "Y-m-d H:i:s"
     * @return array
     */
    public function getLogEntriesByDate( $start, $end ): array
    {
        $range = array(
            $start,
            $end
        );
        return $this->getLogEntriesByParam('created', $range);
    }
    ...
    protected function getLogEntriesByParam( $key, $value )
    {
        ...
        switch($key) {
            ...
            case 'created':
                $start = $value[0];
                $end = $value[1];
                $sql .= " WHERE {$key} >= '{$start}' AND {$key} <= {$end}";
                break;
            default:
                break;
        }
